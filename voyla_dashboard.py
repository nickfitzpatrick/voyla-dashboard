...
with tab1:
    ...
    user_activity = df.groupby(['user_id', 'username']).size().reset_index(name='interactions')
...
with tab2:
    ...
    user_stats.columns = ['user_id', 'total_interactions', 'unique_places', 
                          'unique_properties', 'unique_content']
    
    # Only cluster if we have enough users
    if len(user_stats) < n_clusters:
        st.warning(f"Not enough users ({len(user_stats)}) for {n_clusters} clusters. Showing data without clustering.")
        st.dataframe(user_stats, use_container_width=True)
    else:
        # Perform clustering
        features = ['total_interactions', 'unique_places', 'unique_properties', 'unique_content']
...
with tab3:
    ...
    if df['place_rating'].notna().sum() > 0:
        top_places = df[df['place_rating'].notna()].groupby('place_name').agg({
            'place_rating': 'first',
            'interaction_id': 'count'
        }).rename(columns={'interaction_id': 'interactions'})
        top_places = top_places[top_places['interactions'] >= 2].sort_values('place_rating', ascending=False).head(10)
