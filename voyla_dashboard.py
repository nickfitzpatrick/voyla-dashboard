...
with tab1:
    ...
    with col2:
        ...
        monthly_interactions = df.groupby('interaction_month').size().reset_index(name='count')
        monthly_interactions['month'] = monthly_interactions['interaction_month'].astype(str)
        fig = px.bar(
    ...
    with col1:
        ...
        user_activity = df.groupby(['user_id', 'username']).size().reset_index(name='saves')
...
with tab2:
    ...
    st.markdown("**Business Question:** Should we prioritize power user features or onboarding optimization?")
    ...
    with col1:
        # Pie chart
        colors = {'Power User': '#d62728', 'Casual User': '#ff7f0e', 'New User': '#1f77b4'}
        fig = px.pie(
            segment_stats,
            values='User Count',
            names='Segment',
...
with tab3:
    ...
    st.markdown("**Business Question:** Which market should we expand to next? When should we localize?")
    
    # Calculate active users per country
    country_users = df.groupby('country').agg({
        'user_id': 'nunique',
        'place_id': 'nunique',
    ...
    country_users['% of Total Users'] = (country_users['Active Users'] / df['user_id'].nunique() * 100).round(1)
    ...
    fig.update_traces(marker_color='#2ca02c', texttemplate='%{text:,}', textposition='outside')
    ...
    st.dataframe(
        country_users[['Country', 'Active Users', '% of Total Users', 'Unique Places', 'Total Saves']],
        hide_index=True,
        use_container_width=True
    )
...
with tab4:
    ...
    st.markdown("**Business Question:** What drove growth spikes? Can we replicate them?")
    ...
    interactions_monthly = df.groupby('interaction_month').size().reset_index(name='Total Interactions')
    interactions_monthly['Month'] = interactions_monthly['interaction_month'].astype(str)
    
    # Merge datasets
    growth_data = new_users_monthly.merge(interactions_monthly[['Month', 'Total Interactions']], on='Month', how='left')
    growth_data['Total Interactions'] = growth_data['Total Interactions'].fillna(0).astype(int)
    ...
    with col3:
        st.metric("Peak Growth Month", peak_month['Month'], f"{peak_month['New Users']:,} users")
    with col4:
    ...
    growth_display = growth_data[['Month', 'New Users', 'Cumulative Users', 'Growth Rate', 'Total Interactions']].copy()
    growth_display['Growth Rate'] = growth_display['Growth Rate'].apply(lambda x: f"{x:+.1f}%")
...
with tab5:
    ...
    st.markdown("**Business Question:** When should we trigger re-engagement notifications to prevent churn?")
    ...
    user_activity = df.groupby(['user_id', 'interaction_month']).size().reset_index(name='interactions')
    ...
    if not cohort_df.empty:
        ...
        with col3:
            if avg_m3 > 0:
                st.metric("Avg M3 Retention", f"{avg_m3:.1f}%", f"{avg_m3 - avg_m1:.1f}%")
            else:
                st.metric("Avg M3 Retention", "N/A")
        with col4:
            if avg_m6 > 0:
                st.metric("Avg M6 Retention", f"{avg_m6:.1f}%", f"{avg_m6 - avg_m3:.1f}%")
            else:
